    ...
    >
      ...
      >
        <el-table-column :prop="'id'" :label="'id'" width="80">
          <template #default="scope">
            <el-tooltip placement="right-start">
              ...
              <router-link
                :data-test="`roomList_${roomList[scope.$index].id}`"
                :to="`/${currentUser.role}/${entityPluralName}/${
                  roomList[scope.$index].id
                }`"
        ...
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            <router-link
              :data-test="`roomList${fieldName}EditLink_${
                roomList[scope.$index].id
              }`"
            ...
            <el-button
              :data-test="`roomList${fieldName}DeleteBtn_${
                roomList[scope.$index].id
              }`"
              @click="() => handleOpenConfirmModal(roomList[scope.$index]?.id)"
              icon="Delete"
              circle
              plain
              type="danger"
            ...
            <ConfirmModal
              ...
              :isOpenModal="
                isOpenModal && modalId === roomList[scope.$index].id
              "
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
              ...
              @approve-confirm-modal="
                () => handleRemoveEntity(roomList[scope.$index]?.id)
              "
            >
      ...
      <div class="footer__wrapper">
        <Pagination
          :itemsPerPage="roomPagination.take"
          :set-items-per-page="setItemsPerPage"
          :page="Math.floor(roomPagination.skip / roomPagination.take) + 1"
          :current-page-change="currentPageChange"
          :total="roomPagination.total._count?._all"
...
const entityPluralName = "rooms";
const fieldsName = ref(["patient", "sagesFemmes", "messages"]);
const isOpenModal = ref(false);
const modalId = ref<string | null>(null);
const { isLoading } = storeToRefs(useBodyStore());
const { roomList, roomPagination, roomExcelFile } = storeToRefs(useRoomStore());
const {
  fetchRooms,
  deleteRoom,
  fetchDataExcelRooms,
...
async function dataToExcel() {
  await fetchDataExcelRooms();
  const workbook = XLSX.read(roomExcelFile.value.toString());
...
const currentPageChange = async (val: number) => {
  await fetchRooms({
    skip: (val - 1) * roomPagination.value.take,
    take: roomPagination.value.take,
  });
};
onMounted(async () => {
