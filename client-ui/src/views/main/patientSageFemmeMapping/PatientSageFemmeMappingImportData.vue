    ...
    <el-form :model="form" :rules="rules" ref="ruleFormRef">
      ...
      <div class="form_items">
        ...
        >
          <el-select
            v-model="form[field]"
            :data-test="`patientsagefemmemappingImportData_${field}`"
          >
            <el-option
      ...
      <div class="d-flex justify-content-end">
        <el-button
          data-test="patientsagefemmemappingImportDataFormCancelButton"
          @click="resetForm(ruleFormRef)"
          >Clear</el-button
        >
        <el-button
          data-test="patientsagefemmemappingImportDataFormSubmitButton"
          type="primary"
...
const { error } = storeToRefs(usePatientSageFemmeMappingStore());
const { createManyPatientSageFemmeMapping } = usePatientSageFemmeMappingStore();
...
const patientsagefemmemappingInsertData: any = ref([]);
...
const handelUploadFile: UploadProps["onChange"] = (rawFile: any) => {
  if (rawFile) {
    ...
    reader.onload = (e) => {
      ...
      patientsagefemmemappingInsertData.value = jsonData;
    };
...
const onSubmit = (formEl: InstanceType<typeof ElForm> | undefined) => {
  ...
  formEl.validate(async (valid) => {
    ...
    if (valid && !isEqual(initialState, rest)) {
      patientsagefemmemappingInsertData.value = patientsagefemmemappingInsertData.value.map(
        (item: any) => {
          let res: any = {};
          for (let key in rest) {
      ...
      await createManyPatientSageFemmeMapping(
        patientsagefemmemappingInsertData.value
      );
