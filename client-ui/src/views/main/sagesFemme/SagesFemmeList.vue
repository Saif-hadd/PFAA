  ...
  <div class="mx-auto py-5 ms-4 me-4 h-100">
    ...
    >
      <el-button
        color="#606266"
        class="button__excel"
        :disabled="sagesfemmeList.length === 0"
        @click="dataToExcel"
      ...
      >
        <el-table-column :prop="'id'" :label="'id'" width="80">
          <template #default="scope">
            <el-tooltip placement="right-start">
              ...
              <router-link
                :data-test="`sagesfemmeList_${sagesfemmeList[scope.$index].id}`"
                :to="`/${currentUser.role}/${entityPluralName}/${
                  sagesfemmeList[scope.$index].id
                }`"
        ...
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            <router-link
              :data-test="`sagesfemmeList${fieldName}EditLink_${
                sagesfemmeList[scope.$index].id
              }`"
            ...
            <el-button
              :data-test="`sagesfemmeList${fieldName}DeleteBtn_${
                sagesfemmeList[scope.$index].id
              }`"
              @click="
                () => handleOpenConfirmModal(sagesfemmeList[scope.$index]?.id)
              "
              icon="Delete"
              circle
              plain
            ...
            <ConfirmModal
              ...
              :isOpenModal="
                isOpenModal && modalId === sagesfemmeList[scope.$index].id
              "
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
              ...
              @approve-confirm-modal="
                () => handleRemoveEntity(sagesfemmeList[scope.$index]?.id)
              "
            >
      ...
      <div class="footer__wrapper">
        <Pagination
          :itemsPerPage="sagesfemmePagination.take"
          :set-items-per-page="setItemsPerPage"
          :page="
            Math.floor(sagesfemmePagination.skip / sagesfemmePagination.take) +
            1
          "
          :current-page-change="currentPageChange"
          :total="sagesfemmePagination.total._count?._all"
...
const entityPluralName = "sagesfemmes";
...
const {
  ...
  sagesfemmeExcelFile,
} = storeToRefs(useSagesFemmeStore());
const {
  fetchSagesFemmes,
  deleteSagesFemme,
  fetchDataExcelSagesFemmes,
  softDeleteSagesFemme,
} = useSagesFemmeStore();

async function dataToExcel() {
  await fetchDataExcelSagesFemmes();
  const workbook = XLSX.read(sagesfemmeExcelFile.value.toString());
  XLSX.writeFile(workbook, "sagesfemmeData.xlsx");
}
const deleteLoading = ref<boolean>(false);
const handleRemoveEntity = async (id: string) => {
  deleteLoading.value = true;
  await softDeleteSagesFemme(id);
...
const currentPageChange = async (val: number) => {
  await fetchSagesFemmes({
    skip: (val - 1) * sagesfemmePagination.value.take,
    take: sagesfemmePagination.value.take,
  });
};
onMounted(async () => {
